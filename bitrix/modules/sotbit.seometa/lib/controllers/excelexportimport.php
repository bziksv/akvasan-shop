...
class ExcelExportImport extends Controller
{
    ...
    public static function exportConditionAction(int $offset, int $limit, bool $newFile, $count, $totalCount)
    {
        ...
        try {
            ...
            if (($countRes = $count - $countCHPU) > 0) {
                ...
                $result['PROGRESSBAR'] = SitemapRuntime::showProgress("", Loc::getMessage('SEO_META_RUN_FINISH'), 100);
            }
            return $result;
        } catch (\Exception $e) {
            return $e;
    ...
    public static function exportCHPUAction(int $offset, int $limit, int $newFile, int $count, int $totalCount, array $gridFilter = [])
    {
        require_once($_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/" . self::MODULE_ID . "/vendor/autoload.php");
        require_once($_SERVER["DOCUMENT_ROOT"] . BX_ROOT . "/modules/main/interface/admin_lib.php");

        if ($gridFilter) {
            $filterOptions = new \Bitrix\Main\UI\Filter\Options($gridFilter['ID']);
            $arFilter = $filterOptions->getFilterLogic($gridFilter['FIELDS']);
        }
