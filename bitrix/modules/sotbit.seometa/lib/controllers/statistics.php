...
class Statistics extends Controller
{
                ...
                if ($arCondition && $arrSeometaData = unserialize($arCondition['META'])) {
                    ...
                    if ($section) {
                        ...
                        if ($resSeometaData['META_TITLE']) {
                            $META_TITLE = self::getMeta($resSeometaData['META_TITLE'], $section, $arCondition['INFOBLOCK'], $eventRegions);
                        }
                        if ($resSeometaData['META_KEYWORDS']) {
                            $META_KEYWORDS = self::getMeta($resSeometaData['META_KEYWORDS'], $section, $arCondition['INFOBLOCK'], $eventRegions);
                        }
                        if ($resSeometaData['META_DESCRIPTION']) {
                            $META_DESCRIPTION = self::getMeta($resSeometaData['META_DESCRIPTION'], $section, $arCondition['INFOBLOCK'], $eventRegions);
                        }
                    }
                }
            }
    ...
    private static function getMeta($meta, $section, $infoblock, $eventRegions)
    {
        $result = \CSeoMeta::UserFields($meta, $section, $infoblock);
        if ($eventRegions) {
            ExecuteModuleEventEx($eventRegions[0], [
                &$result
            ]);
        }
        SeoMetaMorphy::init($section, \CSeoMetaTagsProperty::$params);
        $result = SeoMetaMorphy::processMorphy($result);
        $result = Encoding::convertEncodingToCurrent($result);
        return SeoMetaMorphy::convertMorphy($result);
    }
}