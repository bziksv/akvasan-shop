...
//        getList([])->fetchAll();
    ...
    public function Write(
        ...
        $newGenerateCHPU = [
            ...
            'CATEGORY_ID' => $arFields['category_chpu_id'] ?: 0,
    ...
    public function WriteEnd()
    {
        ...
        if((int)self::getAddID() === 1) {
            $xml = file_get_contents($this->dir . 'sitemap_seometa_' . $this->id . '_' . self::getAddID() . '.xml');
            $data = (new XMLMethods)->xml2ary($xml);
            $count = $data['urlset']['_c']['url'] ? count($data['urlset']['_c']['url']) : 0;
            if($count === 0) {
                return false;
            }
        }
