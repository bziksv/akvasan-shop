...
class SeometaUrlTable extends \DataManagerEx_SeoMeta
{
    ...
    public static function addForCopy(array $arr = [])
    {
        return parent::add($arr);
    }

    public static function addAfterParse(?array $arr = [], $mode = ImportHelper::MODE_CREATE)
    {
        $result = null;
        $row = self::getList([
            'select' => [
        ...
        if (in_array($arr['SITE_ID'], $LIDs)) {
            $chpu['SITE_ID'] = $arr['SITE_ID'];
        } else {
            return Loc::getMessage('SEOMETA_GENERATE_SITES_ERROR');
        }

        if ($row) {
            switch ($mode) {
                case ImportHelper::MODE_CREATE:
                    return '<span style="color: red">' . Loc::getMessage('SEO_META_CHPU_EXIST') . '</span>';
                case ImportHelper::MODE_UPDATE:
                    $resultCHPU = parent::update($row['ID'], $arr);
            }
        } else {
        ...
        if (!$resultCHPU->isSuccess()) {
            return '<span style="color: red">' . $resultCHPU->getErrorMessages()[0] . '</span>';
        }
        ...
        if ($row) {
            $updateRow = ChpuSeoDataTable::getByBitrixUrl($arr['REAL_URL'], $arr['SITE_ID']);
            if ($updateRow) {
                $resultCHPUSeo = ChpuSeoDataTable::update($updateRow['ID'], $chpuSeoData);
            }
        }
        ...
        if (!$resultCHPUSeo->isSuccess()) {
            $result = '<span style="color: red">' . $resultCHPUSeo->getErrorMessages()[0] . '</span>';
        }
