...
class ConditionTable extends \DataManagerEx_SeoMeta
{
    ...
    public static function getMap(
        ...
        return [
            ...
            new Entity\IntegerField( 'CATEGORY_CHPU_ID', [
                'required' => true,
                'title' => Loc::getMessage( 'SEOMETA_CATEGORY_CHPU_ID' )
            ]),
			new Entity\TextField( 'TAG', [
					'title' => Loc::getMessage( 'SEOMETA_TAG' )
            ]),
            new Entity\TextField( 'ELEMENT_TAG', [
                'title' => Loc::getMessage( 'SEOMETA_ELEMENT_TAG' )
            ]),
            ...
            new Entity\IntegerField('CATEGORY_TAGS_SECTION_ID', [
                'title' => Loc::getMessage('SEOMETA_CATEGORY_TAGS_SECTION_ID')
            ]),
            new Entity\IntegerField('CATEGORY_TAGS_DETAIL_ID', [
                'title' => Loc::getMessage('SEOMETA_CATEGORY_TAGS_DETAIL_ID')
            ]),
            new Reference(
                'REQUEST',
                AiRequestTable::class,
                Join::on('this.ID', 'ref.CONDITION_ID'),
            ),
            new Reference(
                'CATEGORY_TAGS_SECTION',
                CategoryTagsTable::class,
                Join::on('this.CATEGORY_TAGS_SECTION_ID', 'ref.ID'),
            ),
            new Reference(
                'CATEGORY_TAGS_DETAIL',
                CategoryTagsTable::class,
                Join::on('this.CATEGORY_TAGS_DETAIL_ID', 'ref.ID'),
            ),
		];
	}
    ...
    public static function GetConditionsBySections(
		...
		$Conditions = ConditionTable::getList([
			...
			'select' => [
				...
				'STRICT_RELINKING',
                'HIDE_IN_SECTION',
                'TAGS_SECTION_' => 'CATEGORY_TAGS_SECTION',
                'TAGS_DETAIL_' => 'CATEGORY_TAGS_DETAIL',
            ]
        ]);
    ...
    public static function generateUrlForCondition(
        ...
        if ($arrData && is_array($arrData)) {
            ...
            /*  if ($arrChpuResult && is_array($arrChpuResult) && (count($arrChpu) === count($arrData))) {
                  $arrLinks = ChpuLinksTable::getByMainChpuId(array_keys($arrChpuResult));
                  self::updateChpuLinks($arrLinks, $arrChpuResult);
                  $arrLinks = ChpuLinksTable::getByLinkChpuId(array_keys($arrChpuResult));
                  self::updateChpuLinks($arrLinks, $arrChpuResult);

                  $arrChpuResult = array_flip($arrChpuResult);
              }*/
            foreach ($arrData as $key => $data) {
                $arrChpuLinkUrl[$key] = ['REAL_URL' => $data['REAL_URL'], 'SITE_ID' => $data['SITE_ID']];
            }

            $arrLinks = ChpuLinksTable::query()
                ->setFilter(['MAIN_REAL_URL' => array_column($arrChpuLinkUrl, 'REAL_URL')])
                ->addSelect('*')
                ->fetchCollection();
            foreach ($arrLinks->getAll() as $link) {
                if ($index = array_search(['REAL_URL' => $link->getMainRealUrl(), 'SITE_ID' => $link->getSiteId()], $arrChpuLinkUrl)) {
                    $link->setMainChpuId($index);
                }
            }
            $arrLinks->save();

            $arrLinks = ChpuLinksTable::query()
                ->setFilter(['LINK_REAL_URL' => array_column($arrChpuLinkUrl, 'REAL_URL')])
                ->addSelect('*')
                ->fetchCollection();
            foreach ($arrLinks->getAll() as $link) {
                if ($index = array_search(['REAL_URL' => $link->getLinkRealUrl(), 'SITE_ID' => $link->getSiteId()], $arrChpuLinkUrl)) {
                    $link->setLinkChpuId($index);
                }
            }
            $arrLinks->save();
    ...
    public static function updateChpuLinks(array $arLinks, array $arrChpuResult)
    {
        foreach ($arLinks as $arrLink) {
            $newData = $arrLink;
    ...
    public static function addAfterParse(?array $arr = [], $generate = null)
    {
        $cond = [
            ...
            'CATEGORY_CHPU_ID' => $arr['CATEGORY_CHPU_ID'] ?: 0,
