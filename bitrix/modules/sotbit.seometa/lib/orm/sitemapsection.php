...
class SitemapSectionTable extends \DataManagerEx_SeoMeta
{
    ...
    public static function searchChildSection(array $parentID, $res = [])
    {
        $sections = self::query()
            ->setFilter(['PARENT_CATEGORY_ID' => $parentID])
            ->addSelect('*')
            ->fetchAll();

        if(!$sections) {
            return array_merge($res, $parentID);
        }
        ...
        return self::searchChildSection($parentID, $res);
    }
