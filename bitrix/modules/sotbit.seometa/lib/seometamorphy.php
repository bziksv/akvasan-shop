...
class SeoMetaMorphy
{
    static Section|null $sku = null;
    static \phpMorphy|null $morphyObject = null;
    ...
    public static function morphyLibInit()
    {
        ...
        if (file_exists($fileName) && file_exists($dirName)) {
            ...
            try {
                return new \phpMorphy($dirName, $lang, $opts);
    ...
    public static function prepareForMorphy($metaStr)
    {
        ...
        $pregPatternCondition = Loc::getMessage('PATTERN');
    ...
    public static function convertMorphy($metaString)
    {
            ...
            foreach ($strElements as $strElement) {
                ...
                if (mb_strpos($strElement, "*#") !== false) {
                    ...
                    foreach ($words as $word) {
                        if (self::$morphyObject) {
                            $modifiedWords = self::$morphyObject->castFormByGramInfo(mb_strtoupper($word), null, $modifiers, true);
    ...
    public static function init($sectionId, $properties)
    {
        if (\CSeoMetaTagsProperty::$params !== $properties) {
            \CSeoMetaTagsProperty::$params = $properties;
        }

        if (!self::$sku) {
            self::$sku = new Section($sectionId);
        } elseif ((int)self::$sku->getId() !== $sectionId) {
            self::$sku = new Section($sectionId);
        }


        if (!self::$morphyObject) {
            self::$morphyObject = self::morphyLibInit();
        }
    }

    public static function processMorphy($item)
    {
        return Engine::process(self::$sku, SeoMetaMorphy::prepareForMorphy($item));
    }
}
