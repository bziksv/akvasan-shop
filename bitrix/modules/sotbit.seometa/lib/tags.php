...
class Tags
{
    public function GenerateTags($Conditions = [], $WorkingConditions = [], $countTags = 0, $countTagsPerCond = 0, $elementTag = 0)
    {
        $writer = TagWriter::getInstance($WorkingConditions, $countTags, $elementTag);
            ...
            switch ($Sort) {
                case 'NAME':
                    ...
                    foreach ($Tags as $i => $Tag) {
                        $tmpTags[$i] = trim(mb_strtolower($Tag['TITLE']));
                    }
    ...
    public static function findNeedSections($sections = [], $includeSubsections = 'Y', $includeParent = 'N')
    {
        $sections = !is_array($sections) ? [$sections] : $sections;

        if (in_array($includeSubsections, ['Y', 'A']) || in_array($includeParent, ['Y', 1])) {
            $rsSections = SectionTable::getList([
             ...
             while ($arParentSection = $rsSections->fetch()) {
                 if (in_array($includeSubsections, ['Y', 'A'])) {
                     $subFilter = [
                     ...
                     if ($includeSubsections === 'A') {
                         $subFilter['GLOBAL_ACTIVE'] = 'Y';
                     }

                     $arFilter[0][] = $subFilter;
                 }

                 if (in_array($includeParent, ['Y', 1])) {
                     isset($arFilter) && $arFilter[0]['LOGIC'] = 'OR';
                     $subFilter = [
                         'IBLOCK_ID' => $arParentSection['IBLOCK_ID'],
                         '<LEFT_MARGIN' => $arParentSection['LEFT_MARGIN'],
                         '>RIGHT_MARGIN' => $arParentSection['RIGHT_MARGIN'],
