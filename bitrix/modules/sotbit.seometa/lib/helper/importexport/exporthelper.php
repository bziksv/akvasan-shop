...
class ExportHelper
{
    ...
    public static function headersFilter(array $array, array $entityHeaders)
    {
        ...
        foreach ($array as $key => $item) {
            if (is_object($item) && $item instanceof \Bitrix\Main\ORM\Fields\Relations\Reference) {
                continue;
            }
    ...
    public static function prepareForExportCHPU(array $chpu, array &$headers)
    {
        ...
        if ($chpu['CONDITION_META'] && $condMeta = unserialize($chpu['CONDITION_META'])) {
            ...
            array_walk($condMeta, function ($chp, $key) use (&$headers, &$replace) {
                if ($headers[$key] !== null) {
                    $chp = SeoMetaMorphy::processMorphy($chp);
                    $chp = SeoMetaMorphy::convertMorphy($chp);
    ...
    public static function pathToFile(array $entity, int $offset, array $entityHeaders, array $entityVal, string $sheetName = 'seometa_condition')
    {
        ...
        foreach ($worksheet->getColumnIterator() as $column) {
            ...
            if (in_array($value, $arrHeadersName) && ($offset === 0 || $offset - 2 === 0)) {
                if (str_starts_with($arrHeaders[$value], '*')) {
                    $firstRowComment = substr($arrHeaders[$value], 1);
                    $worksheet->getStyle($firstRow)->getFont()->setBold(true);
                } else {
                    $firstRowComment = $arrHeaders[$value];
                }
                $worksheet->getComment($firstRow)->getText()->createTextRun($firstRowComment);
                $secondRowComment = Encoding::convertToUtf(Loc::getMessage('SEOMETA_EXPORTHELPER_' . $langVal . '_EXAMPLE_' . $value));
                if (!empty($secondRowComment)) {
                    $worksheet->getComment($secondRow)->getText()->createTextRun($secondRowComment);
                    $worksheet->getComment($secondRow)->setWidth('250');
                    $worksheet->getComment($secondRow)->setHeight('100');
                }

                if ($value === 'ID') {
                    $worksheet->getColumnDimension($column->getColumnIndex())->setVisible(false);
                }
        ...
        $date = (new Type\DateTime())->toString();
        $date = str_replace(['.', ' ', ':', '/', '\\'], '_', $date);

        return [
            'NAME' => $sheetName . '_' . $date . ".xlsx",
            'PATH' => $path
        ];
    }
}