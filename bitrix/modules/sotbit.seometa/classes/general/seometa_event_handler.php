...
class CSeoMetaEvents
{
    ...
    public static function PageStart() {
        global $APPLICATION, $PAGEN_1, $USER;
        $USER = new \CUser;
        ...
        $excludeParams = Option::get(self::MODULE_NAME, 'PARAMS_EXCEPTION_SETTINGS', '', SITE_ID);
        ...
        if ($context->getRequest()->isAjaxRequest()
            && !$context->getRequest()->get('ajaxTag')
            && Option::get(self::MODULE_NAME, 'RETURN_AJAX_'. SITE_ID, 'N', SITE_ID) == 'Y'
        ) {
            return;
        }
        ...
        $str = Option::get("sotbit.seometa", 'PAGENAV_' . SITE_ID, '', SITE_ID);
        ...
        if ($instance && ($instance['NEW_URL'] != $instance['REAL_URL']) && SITE_ID == $instance['SITE_ID']) {
            ...
            // #67230: fix ajax form when sending leads to 404
            ...
            $context->initialize(
                new Bitrix\Main\HttpRequest(
                    $server,
                    $_GET,
                    $_POST, // #67230: fix ajax form when sending leads to 404
                    [],
                    $_COOKIE
                ),
                $context->getResponse(),
    ...
    static function EpilogAfter()
    {
        if(($_REQUEST && ($_REQUEST['SEOMETA_STATUS_AGENT'] === 'Y' && $_REQUEST['SEOMETA_STATUS_CODE'] === 'Y')) || self::$googleBot || self::$yandexBot){
            ...
            list($valueSeoMetaTitle, $valueSeoMetaKeywords, $valueSeoMetaDescription) = [$sotbitSeoMetaTitle, $sotbitSeoMetaKeywords, $sotbitSeoMetaDescription];
            ...
            $eventRegions = \Bitrix\Main\EventManager::getInstance()->findEventHandlers("main", "OnEndBufferContent", ['TO_MODULE' => 'sotbit.regions']);
            if ($eventRegions) {
                $currentTitle = self::getRegionsTemp($eventRegions, $currentTitle);
                $currentKeywords = self::getRegionsTemp($eventRegions, $currentKeywords);
                $currentDescription = self::getRegionsTemp($eventRegions, $currentDescription);
                $valueSeoMetaTitle = self::getRegionsTemp($eventRegions, $valueSeoMetaTitle);
                $valueSeoMetaKeywords = self::getRegionsTemp($eventRegions, $valueSeoMetaKeywords);
                $valueSeoMetaDescription = self::getRegionsTemp($eventRegions, $valueSeoMetaDescription);
            }
            $seoResult['META_TITLE']['COINCIDENCE'] = $valueSeoMetaTitle == $currentTitle ? 'Y' : 'N';
            $seoResult['META_TITLE']['CONTENT'] = $currentTitle;
            $seoResult['META_KEYWORDS']['COINCIDENCE'] = $valueSeoMetaKeywords == $currentKeywords ? 'Y' : 'N';
            $seoResult['META_KEYWORDS']['CONTENT'] = $currentKeywords;
            $seoResult['META_DESCRIPTION']['COINCIDENCE'] = $valueSeoMetaDescription == $currentDescription ? 'Y' : 'N';
    ...
    public static function getRegionsTemp($eventRegions, $temp) {
        ExecuteModuleEventEx($eventRegions[0], [
            &$temp
        ]);
        return $temp;
    }
    ...
    public static function GenerateSeoMetaSitemap()
    {
        global $USER;
        if(!$USER) {
            $USER = new \CUser;
        ...
        $sitemap = \Sotbit\Seometa\Orm\SitemapTable::query()
            ->setFilter(['SITE_ID' => SITE_ID])
            ->addSelect('*')
            ->setOrder(['DATE_RUN' => 'DESC'])
            ->fetch();

       if($settings = unserialize($sitemap['SETTINGS'])) {
           if($settings['AGENT'] !== 'Y') {
                if($settings['GENERATE_ALL_CONDITIONS'] === 'Y') {
                    SiteMapGenerate::generateHeavy($sitemap['ID']);
                } else {
                    SiteMapGenerate::generateLight($sitemap['ID']);
                }
           }
       }
    }
