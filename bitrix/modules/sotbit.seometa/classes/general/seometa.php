...
class CSeoMeta extends Bitrix\Iblock\Template\Functions\FunctionBase {
    ...
    static function GetValuesForEnum_NoCache(
        ...
        } elseif ($enum_fields["PROPERTY_TYPE"] === "G") {
            $rsLink = \Bitrix\Iblock\SectionTable::query()
                ->setFilter(['ID' => $Xvalue, 'IBLOCK_ID' => $enum_fields['LINK_IBLOCK_ID']])
                ->setSelect(['ID', 'NAME', 'DEPTH_LEVEL'])
                ->exec();

            while ($sectionFields = $rsLink->fetch()) {
                $depthLevel = str_repeat('.', $sectionFields['DEPTH_LEVEL']);
                $ListValues[$sectionFields['ID']] = $depthLevel . $sectionFields['NAME'];
            }
        }
    ...
    public function getSections($iblockId) {
        ...
        $arrSectionsList = [
            'REFERENCE' => [
               Loc::getMessage('SEOMETA_SECTION_TOP_LEVEL')
            ],
            'REFERENCE_ID' => [
                0
            ],
