...
while ($arRes = $rsData->NavNext(true, "")) {
    ...
    $arActions = [];
    if ($arRes['T'] == 'P') {
        $row->AddViewField("PROPERTIES", \Sotbit\Seometa\Helper\AdminSection\Tools::getPropertyChpuValue($arRes['PROPERTIES'], $arRes['iblock_id']));
        ...
        $row->AddViewField("NAME", '<a target="_blank" href=\'sotbit.seometa_chpu_edit.php?ID='.$arRes['ID'].'&lang='.LANG.$ParentUrl.'\'>'.$arRes['NAME'].'</a>');
        $row->AddViewField("SORT", $arRes['SORT']);
        $arActions[] = [
            "ICON"=>"edit",
            "DEFAULT"=>true,
            "TEXT"=>GetMessage("SEO_META_EDIT"),
            "ACTION" => htmlspecialcharsbx("window.open('sotbit.seometa_chpu_edit.php?ID=" . $arRes['ID'] . "&lang=" . LANG . $ParentUrl . "', '_blank');"),
        ];
    }
