...
$lAdmin = new \Sotbit\Seometa\Helper\AdminSection\AdminUIList($sTableID, $oSort);
...
$lAdmin->SetNavigationParams($rsData, array("BASE_LINK" => '/bitrix/admin/sotbit.seometa_sitemap_list.php'));
...
$arContext = [
    [
        ...
        "TITLE" => Loc::getMessage("SEO_META_SEO_ADD_SITEMAP"),
    ]
];
...
foreach ($arSites as $arRes) {
    $arContext[] = [
        ...
        "TITLE" => "[" . $arRes["LID"] . "] " . $arRes["NAME"],
        "LINK" => "sotbit.seometa_sitemap_edit.php?lang=" . LANGUAGE_ID . "&site_id=" . $arRes['LID']
    ];
}

$lAdmin->AddAdminContextMenu($arContext, false, false);
$lAdmin->AddGroupActionTable(["delete" => Loc::getMessage("MAIN_ADMIN_LIST_DELETE"), "for_all" => true]);
...
?>
<style>
    #bx-admin-prefix .bx-core-popup-menu-no-icons a .bx-core-popup-menu-item-text
    {
        padding-left: 40px;
    }

    #bx-admin-prefix a .bx-core-popup-menu-item-icon {
    ...
    function generateSitemapLight (ID) {
        ...
        BX.ajax.post('/bitrix/admin/sotbit.seometa_sitemap_light_run.php', {
            ...
            if(data.search(/errortext/) !== -1) {
                BX('sitemap_run').style.display = 'none';
                document.querySelector('#show-error-seometa .adm-info-message .adm-info-message-title').innerHTML = data;
                BX('show-error-seometa').style.display = 'block';
            } else {
                showStatus(data);
            }
        });
        ...
        BX.ajax.post('/bitrix/admin/sotbit.seometa_sitemap_light_run.php', {
            ...
            if((result['STATUS'] !== undefined && result['STATUS'] === 'finish') || result === undefined) {
                if(result['TYPE'] === 'ERROR') {
                    BX('sitemap_run').style.display = 'none';
                    document.querySelector('#show-error-seometa .adm-info-message .adm-info-message-title').innerHTML = result['MSG'];
                    BX('show-error-seometa').style.display = 'block';
                }
            } else {
                setTimeout(function () {
    ...
    BX.runSitemap = function (ID, value, pid, NS, iteration = 0, currCond = 0, currSection = 0, params = []) {
        ...
        BX.ajax.post('/bitrix/admin/sotbit.seometa_sitemap_run.php', {
            ...
            if(IsJsonString(data) !== false){
                ...
                BX('show-error-seometa').style.display = 'none';
            }else{
