...
$aiInc = Option::get(\CCSeoMeta::MODULE_ID, 'AI_FUNC_INC') === 'Y';
$aTabs = [
    [
        "DIV" => "edit1",
    ...
    [
        "DIV" => "edit7",
        "TAB" => GetMessage("SEO_META_EDIT_TAB_TAGS"),
        "ICON" => "main_user_edit",
        "TITLE" => GetMessage("SEO_META_EDIT_TAB_TAGS_TITLE")
    ],
    [
        "DIV" => "edit3",
...
if ($aiInc) {
    $aTabs[] = [
        "DIV" => "edit6",
        "TAB" => GetMessage("SEO_META_EDIT_AI_GENERATE"),
        "ICON" => "main_user_edit",
        "TITLE" => GetMessage("SEO_META_EDIT_AI_GENERATE")
    ];
}
...
if (!empty($_REQUEST['action'])) {
    ...
    if ($_REQUEST['action'] == "copy" && !empty($ID)) {
        ...
        $arFields = [
            ...
            "CATEGORY_CHPU_ID" => $condition['CATEGORY_CHPU_ID']
        ];
...
if ($REQUEST_METHOD == "POST" && ($save != "" || $apply != "") && $POST_RIGHT == "W" && check_bitrix_sessid()) {
    ...
    $arFields = [
        ...
        "CATEGORY_CHPU_ID" => $CATEGORY_CHPU_ID,
        "SORT" => $SORT,
        ...
        "TAG" => $TAG,
        "CATEGORY_TAGS_SECTION_ID" => $CATEGORY_TAGS_SECTION ?: 1,
        "ELEMENT_TAG" => $ELEMENT_TAG,
        "CATEGORY_TAGS_DETAIL_ID" => $CATEGORY_TAGS_DETAIL ?: 1,
    ...
    if (!empty($ID)) {
        ...
        if ($_POST['AI_CLEAR_REQUEST'] === 'Y') {
            $aiRequestEntity = \Sotbit\Seometa\Orm\AiRequestTable::query()->setFilter(['CONDITION_ID' => $idForAi])->addSelect('ID')->fetch();
            \Sotbit\Seometa\Orm\AiRequestTable::delete($aiRequestEntity['ID']);
        }
    } else {
        $result = ConditionTable::add($arFields);
        if ($result->isSuccess()) {
    ...
    if(!empty($ID) && !$errors) {
        ...
        if(is_array($ogSettings['OG_FIELD_IMAGE']) || $delOg = $ogSettings['OG_FIELD_IMAGE_del'] == 'Y') {
            $ogSettings['OG_FIELD_IMAGE'] = OpengraphTable::saveFile(['OG_FIELD_IMAGE' => $ogSettings['OG_FIELD_IMAGE']], $delOg);
        }
        ...
        if(is_array($twSettings['TW_FIELD_IMAGE']) || $delTw = $twSettings['TW_FIELD_IMAGE_del'] == 'Y') {
            $twSettings['TW_FIELD_IMAGE'] = OpengraphTable::saveFile(['TW_FIELD_IMAGE' => $twSettings['TW_FIELD_IMAGE']], $delTw);
        }
...
if ($REQUEST_METHOD === 'POST' && ($_REQUEST['refresh_infoblock_id'] || $_REQUEST['refresh_iblock_type'])) {
    $condition['RULE'] = [];
}
...
$tabControl->BeginCustomField("CATEGORY_ID", GetMessage('SEO_META_EDIT_CATEGORY_ID'));
?>
    <tr id="CATEGORY_ID">
        ...
        <td width="60%">
            <select class="js-select2" name="CATEGORY_ID">
                ...
                foreach ($RsAllSections as $section) {
                    $selected = '';
                    if ($condition['CATEGORY_ID'] === $section['ID']) {
                        $selected = 'selected';
                    } elseif (!$condition['CATEGORY_ID'] && $_REQUEST['section'] === $section['ID']) {
                        $selected = 'selected';
                    } ?>
...
$tabControl->BeginCustomField("CATEGORY_TAGS_SECTION", GetMessage('SEO_META_EDIT_CATEGORY_TAGS_SECTION'));
?>
    <tr id="CATEGORY_TAGS_SECTION">
        <td width="40%">
            <div id="form-control-hint">
                <span data-hint="<?= Loc::getMessage('SEO_META_CHECK_CATEGORY_TAGS_HINT') ?>" data-hint-html></span>
                <?= $tabControl->GetCustomLabelHTML(); ?>
            </div>
        </td>
        <td width="60%">
            <select class="js-select2" name="CATEGORY_TAGS_SECTION">
                <option value="0"><?= GetMessage("SEO_META_CHECK_CATEGORY_TAGS") ?></option>
                <?
                $categoryTags = \Sotbit\Seometa\Orm\CategoryTagsTable::getList([
                    'select' => ['*'],
                    'filter' => ['ACTIVE' => 'Y'],
                    'order' => ['SORT' => 'ASC']
                ])->fetchAll();
                foreach ($categoryTags as $section) {
                    $selected = '';
                    if ($condition['CATEGORY_TAGS_SECTION_ID'] === $section['ID']) {
                        $selected = 'selected';
                    } ?>
                    <option value='<?= $section['ID'] ?>' <?= $selected ?>><?= $section['NAME'] ?></option>
                <? } ?>
...
<?
$tabControl->EndCustomField("CATEGORY_TAGS_SECTION");
...
$tabControl->BeginCustomField("CATEGORY_TAGS_SECTION_NOTE",
    ""
);
?>
    <tr>
        ...
        <td>
            <?= BeginNote(); ?>
            <?= Loc::getMessage("SEO_META_CHECK_CATEGORY_TAGS_HINT") ?>
            <?= EndNote(); ?>
        </td>
    </tr>
<?
$tabControl->EndCustomField("CATEGORY_TAGS_SECTION_NOTE");
...
$tabControl->BeginCustomField("CATEGORY_TAGS_DETAIL", GetMessage('SEO_META_EDIT_CATEGORY_TAGS_DETAIL'));
?>
    <tr id="CATEGORY_TAGS_DETAIL">
        <td width="40%">
            <div id="form-control-element-hint">
                    <span data-hint="<?= Loc::getMessage('CATEGORY_TAGS_DETAIL_HINT') ?>" data-hint-html></span>
                <?= $tabControl->GetCustomLabelHTML(); ?>
            </div>
        </td>
        <td width="60%">
            <select class="js-select2" name="CATEGORY_TAGS_DETAIL">
                <option value="0"><?= GetMessage("SEO_META_CHECK_CATEGORY_TAGS") ?></option>
                <?
                $categoryTags = \Sotbit\Seometa\Orm\CategoryTagsTable::getList([
                    'select' => ['*'],
                ...
                foreach ($categoryTags as $section) {
                    $selected = '';
                    if ($condition['CATEGORY_TAGS_DETAIL_ID'] === $section['ID']) {
                        $selected = 'selected';
                    } ?>
                    <option value='<?= $section['ID'] ?>' <?= $selected ?>><?= $section['NAME'] ?></option>
                <? } ?>
...
<?
$tabControl->EndCustomField("CATEGORY_TAGS_DETAIL");

$tabControl->BeginCustomField("ELEMENT_TAG", GetMessage('SEO_META_EDIT_ELEMENT_TAG'));
?>
    <tr class="adm-detail-valign-top">
        <td width="40%">
            <?= $tabControl->GetCustomLabelHTML(); ?>
        ...
        <textarea
                style="width: 90%"
                name="ELEMENT_TAG"><?= $condition['ELEMENT_TAG'] ?: '' ?></textarea>
        </td>
        <td width="10%"
            align="left">
            <?= $PropMenu; ?>
...
<? $tabControl->EndCustomField("ELEMENT_TAG");

$tabControl->BeginCustomField("CATEGORY_TAGS_DETAIL_NOTE",
    ""
);
?>
    <tr>
        ...
        <td>
            <?= BeginNote(); ?>
            <?= Loc::getMessage("SEO_META_CHECK_CATEGORY_TAGS_DETAIL_HINT") ?>
            <?= EndNote(); ?>
        </td>
    </tr>
<?
$tabControl->EndCustomField("CATEGORY_TAGS_DETAIL_NOTE");
...
$tabControl->BeginCustomField("CATEGORY_CHPU_ID", GetMessage('SEO_META_EDIT_CATEGORY_CHPU_ID'));
?>
    <tr id="CATEGORY_CHPU_ID">
        <td width="40%"><?= $tabControl->GetCustomLabelHTML(); ?></td>
        <td width="60%">
            <select class="js-select2" name="CATEGORY_CHPU_ID">
                <option value="0"><?= GetMessage("SEO_META_CHECK_CATEGORY") ?></option>
                <?
                $RsAllSectionsCHPU = SectionUrlTable::getList([
                    'select' => ['*'],
                    'filter' => ['ACTIVE' => 'Y'],
                    'order' => ['SORT' => 'ASC']
                ])->fetchAll();
                foreach ($RsAllSectionsCHPU as $section) {
                    $selected = '';
                    if ($condition['CATEGORY_CHPU_ID'] === $section['ID']) {
                        $selected = 'selected';
                    } ?>
                    <option value='<?= $section['ID'] ?>' <?= $selected ?>><?= $section['NAME'] ?></option>
                <? } ?>
...
<?
$tabControl->EndCustomField("CATEGORY_CHPU_ID");
...
//OpenGraph
$tabControl->BeginCustomField("OG_GROUP_ELEMENT", Loc::getMessage('SEO_META_EDIT_OG_GROUP_ELEMENT'));
?>
    <tr class="heading">
        <td colspan="3"><?= $tabControl->GetCustomLabelHTML(); ?></td>
    </tr>
<? $tabControl->EndCustomField("OG_GROUP_ELEMENT");
...
//TwitterCard
$tabControl->BeginCustomField("TW_GROUP_ELEMENT", Loc::getMessage('SEO_META_EDIT_TW_GROUP_ELEMENT'));
?>
    <tr class="heading">
        <td colspan="3"><?= $tabControl->GetCustomLabelHTML(); ?></td>
    </tr>
<? $tabControl->EndCustomField("TW_GROUP_ELEMENT");
...
if ($aiInc) {
    ...
    $tabControl->BeginNextFormTab();
    if($idForAi > 0) {
        $aiRequestInfo = \Sotbit\Seometa\Orm\AiRequestTable::query()->setFilter(['CONDITION_ID' => $idForAi])->addSelect('*')->fetch();
    } else {
        $aiRequestInfo = [];
    }
    $tabControl->AddTextField('AI_INPUT_REQUEST',
        Loc::getMessage('SEO_META_AI_INPUT_REQUEST'),
        $_REQUEST['AI_INPUT_REQUEST'] ?: $aiRequestInfo['SEND_REQUEST'],
        [
            'textarea' => [
    ...
    $tabControl->BeginCustomField("AI_OUTPUT_REQUEST", Loc::getMessage('SEO_META_AI_OUTPUT_REQUEST')); ?>
    <tr id="AI_OUTPUT_REQUEST">
        <td width="40%"><?= $tabControl->GetCustomLabelHTML(); ?></td>
        <td width="60%">
            <div data-name="AI_OUTPUT_REQUEST">
                <?= $aiRequestInfo['OUTPUT_REQUEST'] ?>
            </div>
        </td>
    </tr>
    <?
    $tabControl->EndCustomField("AI_OUTPUT_REQUEST");

    $tabControl->BeginCustomField("AI_BUTTON_SEND", '');
    ?>
    <tr>
        <td></td>
        <td id="btn_section_send_request" style="position: relative">
            <input class="adm-btn-save" id="send_ai_request" name="send_ai_request" type="submit"
                   value="<?= GetMessage('SEO_META_AI_SEND_REQUEST') ?>">
            <input id="send_ai_request_reset" type="button" value="<?= GetMessage('SEO_META_AI_SEND_REQUEST_RESET') ?>">
        </td>
    </tr>
    <?
    $tabControl->EndCustomField("AI_BUTTON_SEND");
    ?>
    <script>
        BX.ready(function () {
            const sendAiRequest = document.querySelector('#send_ai_request');
            const textAreaOutputRequest = document.querySelector('div[data-name="AI_OUTPUT_REQUEST"]');
            const textAreaInputRequest = document.querySelector('textarea[name="AI_INPUT_REQUEST"]');
            const reset = document.querySelector('#send_ai_request_reset');
            reset.addEventListener('click', function (e) {
                textAreaOutputRequest.innerText = '';
                textAreaInputRequest.value = '';
                const inp = document.createElement('input');
                inp.type = 'hidden';
                inp.name = 'AI_CLEAR_REQUEST';
                inp.value = 'Y';
                textAreaOutputRequest.appendChild(inp)
            })

            document.addEventListener('DOMContentLoaded', function () {
                textAreaInputRequest.addEventListener('input', function () {
                    this.style.height = "66px";
                    this.style.height = (this.scrollHeight + 6) + "px";
                });
            });
            sendAiRequest.addEventListener('click', function (e) {
                e.preventDefault();
                const btnSection = document.querySelector('#btn_section_send_request');
                const loader = document.createElement("div");
                loader.classList.add('adm-btn-load-img-green');
                loader.style.top = 12 + 'px';
                loader.style.left = 46 + 'px';
                btnSection.appendChild(loader);
                this.classList.add('adm-btn-load');

                BX.ajax.runAction('sotbit:seometa.airequest.sendRequest', {
                    data: {
                        id: '<?= $idForAi ?>',
                        message: textAreaInputRequest.value,
                    }
                }).then(response => {
                        if (response.data.ANSWER) {
                            textAreaOutputRequest.innerText = response.data.ANSWER;
                            textAreaOutputRequest.style.color = "";
                            btnSection.removeChild(loader);
                            sendAiRequest.classList.remove('adm-btn-load');
                            const inp = document.querySelector('input[name="AI_CLEAR_REQUEST"]');
                            if (inp) {
                                inp.remove();
                            }
                        }
                        ...
                        btnSection.removeChild(loader);
                        textAreaOutputRequest.innerText = error.errors[0].message;
                        textAreaOutputRequest.style.color = "red";
                        sendAiRequest.classList.remove('adm-btn-load');
                    }
                );
            });
...
?>
    ...
    <tr class="adm-detail-valign-top">
        <td colspan="2"
            align="center">
            <iframe width="720" height="405" src="https://rutube.ru/play/embed/c4cd1676f1ce6e964c840358eb039e26/" style="border: none;" allow="clipboard-write; autoplay" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>
        </td>
    </tr>
...
if ($aiInc && $_REQUEST['tabControl_active_tab'] === 'edit2') {
    $tabControl->Buttons($arButtonsParams, '<input type="submit" onclick="" style="display: inline" name="generate" value="' . Loc::getMessage("SEO_META_GENERATE_AI_META_TAGS") . '" />');
} elseif ($_REQUEST['tabControl_active_tab'] !== 'edit2') {
    $tabControl->Buttons($arButtonsParams, '<input type="submit" onclick="" style="display: none" name="generate" value="' . Loc::getMessage("SEO_META_GENERATE_AI_META_TAGS") . '" />');
} else {
...
$resArSeoKeys = [];
foreach ($arSeoKeys as $val) {
    $resArSeoKeys[$val] = [
        'name' => 'checkMeta[' . $val . ']',
        'type' => 'checkbox',
        'title' => Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_META_' . $val),
        'value' => $val,
    ];
}
$resArSeoKeys['KEYWORDS_FOR_REQUEST'] = [
    'name' => 'KEYWORDS_FOR_REQUEST',
    'type' => 'text',
    'title' => Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_META_KEYWORDS_FOR_REQUEST'),
    'value' => '',
];
...
if ($aiInc) {
    ?>
    <script>
        MessageForRequest = function () {
            this.arSeo = {
                'TITLE': document.querySelector('#ELEMENT_TITLE'),
                'KEYWORDS': document.querySelector('#ELEMENT_KEYWORDS'),
                'DESCRIPTION': document.querySelector('#ELEMENT_DESCRIPTION'),
                'H1': document.querySelector('#ELEMENT_PAGE_TITLE'),
                'UPPER_DESCRIPTION': document.querySelector('[name="ELEMENT_TOP_DESC"]'),
                'LOWER_DESCRIPTION': document.querySelector('[name="ELEMENT_BOTTOM_DESC"]'),
                'ADDITIONAL_DESCRIPTION': document.querySelector('[name="ELEMENT_ADD_DESC"]'),
            }

            this.generateMeta = document.querySelector('input[name="generate"]');
            this.message = '';
        }
        BX.ready(function () {
            const btnTabs = document.querySelector('#tabControl_tabs');
            btnTabs.addEventListener('click', function (e) {
                if(e.target.id === 'tab_cont_edit2') {
                    window.messageforrequest.generateMeta.style.display = 'inline';
                } else {
                    window.messageforrequest.generateMeta.style.display = 'none';
                }
            });

            window.messageforrequest.generateMeta.addEventListener('click', function (e) {
                e.preventDefault();
                const arSeo = window.messageforrequest.arSeo;
                Object.keys(arSeo).forEach((i) => {
                    arSeo[i].style.border;
                })
                BX.UI.StepProcessing.ProcessManager.create(
                    <?=Json::encode([
                        'id' => 'generateMetaInformation',
                        'controller' => 'sotbit:seometa.controllers.airequest',
                        'optionsFields' => $resArSeoKeys,
                    ])?>).setQueue([
                    {
                        'action': 'generateMeta',
                        'title': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS') ?>',
                        'handlers': {
                            'StepCompleted': function (state, data) {
                                if (state === BX.UI.StepProcessing.ProcessResultStatus.completed) {
                                    window.messageforrequest.message = data['COMPLETED_MESSAGE'];
                                    window.messageforrequest.checkMeta = data['CHECK_META'];
                                    BX.UI.StepProcessing.ProcessManager.create(
                                        {
                                            id: 'completedRequest',
                                            controller: 'sotbit:seometa.controllers.airequest',
                                            optionsFields: {
                                                TEXT: {
                                                    name: 'finalMessage',
                                                    type: 'text',
                                                    title: '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_META_TITLE_READY_MESSAGE') ?>',
                                                    value: window.messageforrequest.message,
                                                },
                                            },
                                        }).setQueue([
                                        {
                                            'action': 'sendRequestWithMessage',
                                            'title': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS') ?>',
                                            'handlers': {
                                                'StepCompleted': function (state, data) {
                                                    if (state === BX.UI.StepProcessing.ProcessResultStatus.completed) {
                                                        ...
                                                        let completedRequest = document.querySelector('#completedRequest');
                                                        let observer = new IntersectionObserver(function (entries, observer) {
                                                            if(!entries[0].isIntersecting) {
                                                                completedRequest.style.top = 421 + 'px';
                                                                observer.unobserve(completedRequest);
                                                            }
                                                        }, options);
                                                        observer.observe(completedRequest);
                                                        window.scroll({
                                                            top: 0,
                                                            left: 0,
                                                            behavior: 'smooth'
                                                        ...
                                                        if(data) {
                                                            Object.keys(data).forEach((i) => {
                                                                if(arSeo[i] && data[i]) {
                                                                    arSeo[i].value = data[i];
                                                                    arSeo[i].style.border = '2px #729e00 solid';
                                                                }
                                                            })
                                                        }
                                        ...
                                        'choosingMeta': window.messageforrequest.checkMeta,
                                    }).setMessages({
                                        'DialogTitle': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_TITLE') ?>',
                                        'DialogSummary': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_SUMMARY') ?>',
                                        'DialogStartButton': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_START') ?>',
                                        'DialogStopButton': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_STOP') ?>',
                                        'DialogCloseButton': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_CLOSE') ?>',
                                        'RequestCanceling': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_CANCELING') ?>',
                                        'RequestCanceled': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_CANCELED') ?>',
                                        'RequestCompleted': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_COMPLETED') ?>',
                                    });
                                    BX.UI.StepProcessing.ProcessManager.get('completedRequest').showDialog();
                                    let requestTexArea = document.querySelector('#completedRequest_opt_finalMessage');
                                    requestTexArea.style.height = (requestTexArea.scrollHeight + 6)+"px";
                                }
                            }
                        }
                    },
                    ...
                    'DialogTitle': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_GENERATE_TITLE') ?>',
                    'DialogSummary': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_GENERATE_SUMMARY') ?>',
                    'DialogStartButton': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_GENERATE_START') ?>',
                    'DialogStopButton': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_STOP') ?>',
                    'DialogCloseButton': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_CLOSE') ?>',
                    'RequestCanceling': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_CANCELING') ?>',
                    'RequestCanceled': '<?= Loc::getMessage('SEO_META_GENERATE_AI_META_TAGS_CANCELED') ?>',
                ...
                BX.UI.StepProcessing.ProcessManager.get('generateMetaInformation').showDialog();
            });
        });

        window.messageforrequest = new MessageForRequest();
    </script>
    <style>
        .bx-stepprocessing-dialog-process-options .bx-stepprocessing-dialog-process-options-input textarea{
            white-space: unset;
        }
    </style>
    <?
