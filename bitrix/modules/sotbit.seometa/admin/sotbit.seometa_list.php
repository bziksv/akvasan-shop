...
if ($arFilter) {
    $arFilter['CATEGORY_ID'] = SitemapSectionTable::searchChildSection([$parentID]);
} else {
...
if ($parentID > 0) {
    ...
    $sectionUrl = "/bitrix/admin/sotbit.seometa_list.php?lang=". LANG . "&parent=". $Section['PARENT_CATEGORY_ID'];
    $showField = "<a href=". $sectionUrl ."><span class=\"adm-submenu-item-link-icon fileman_icon_folder_up\" alt=\"".Loc::getMessage("SEO_META_SECTION_UP")."\"></span>&nbsp;<a href=\"sotbit.seometa_list.php?lang=". LANG . "&parent=". $Section['PARENT_CATEGORY_ID'] ."\">..</a>";
...
$lAdmin->SetNavigationParams($rsData, array("BASE_LINK" => '/bitrix/admin/sotbit.seometa_list.php'));
while($arRes = $rsData->NavNext(true, "f_")) {
    $row =& $lAdmin->AddRow($f_T . $f_ID, $arRes);
    $row->AddInputField("NAME", array("size" => 20));
    if ($f_T == "S") {
        $sectionUrl = "/bitrix/admin/sotbit.seometa_list.php?parent=" . $f_ID . '&lang=' . LANG;
        $row->AddViewField("NAME",
            '<a href="'. (new \Bitrix\Main\Web\Uri($sectionUrl))->toAbsolute() .'" class="adm-list-table-icon-link" title="' . Loc::getMessage("IBLIST_A_LIST") . '"><span class="adm-submenu-item-link-icon adm-list-table-icon iblock-section-icon"></span><span class="adm-list-table-link">' . $f_NAME . '</span></a>');
    } else {
        $row->AddViewField("NAME",
    ...
    if ($f_T == 'P') {
        ...
        $arActions[] = [
            "ICON" => "move",
            "TEXT" => Loc::getMessage("SEO_META_RUN"),
    ...
    if ($POST_RIGHT >= "W") {
        $arActions[] = [
            ...
            "ACTION" => "if(confirm('" . Loc::getMessage('SEO_META_DEL_CONF') . "')) " . $lAdmin->ActionDoGroup($f_T . $f_ID,
                    "delete")
        ];
    }
...
$lAdmin->AddFooter(
	[
		["title"=>Loc::getMessage("SEO_META_LIST_SELECTED"), "value"=>$rsData->SelectedRowsCount()],
		["counter"=>true, "title"=>Loc::getMessage("SEO_META_LIST_CHECKED"), "value"=>"0"],
    ]
);
