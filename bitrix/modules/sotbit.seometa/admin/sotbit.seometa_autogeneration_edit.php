...
if ($ID > 0) {
    ...
    if($condition["META"]) {
        $meta = unserialize($condition["META"], ['allowed_classes' => false]);
    }
...
if ($condition["INFOBLOCK"]) {
    $arrSectionsList = (new CSeoMeta())->getSections($condition["INFOBLOCK"]);
...
if ($condition["RULE"]) {
    if (!is_array($condition["RULE"])) {
        $condition["RULE"] = unserialize($condition["RULE"], ['allowed_classes' => false]);
    }
...
if ($REQUEST_METHOD == "POST" && ($save != "" || $apply != "") && $POST_RIGHT == "W" && check_bitrix_sessid()) {
     ...
     if(!$_REQUEST['META']['CATEGORY_TAGS_SECTION']) {
         $_REQUEST['META']['CATEGORY_TAGS_SECTION'] = 1;
     }

    if(!$_REQUEST['META']['CATEGORY_TAGS_DETAIL_SECTION']) {
        $_REQUEST['META']['CATEGORY_TAGS_DETAIL_SECTION'] = 1;
    }
...
if ($_REQUEST['action'] == "start") {
    $done = false;
    if ($condition['RULE'] && $condition['SECTIONS'] !== 'N;') {
    ...
    if ($condition['SECTIONS'] === 'N;') {
        $errors[] = Loc::getMessage("SEOMETA_EMPTY_SECTIONS");
    }
}
//</editor-fold>
...
<form method="POST" action="<?=$APPLICATION->GetCurPage()?>" enctype="multipart/form-data" name="post_form">
    ...
    <tr>
        ...
        <td>
            <?=CSite::SelectBoxMulti("SITES", !isset($condition['SITES']) ? $ag->getSites() : (is_array($condition['SITES']) ? $condition['SITES'] : unserialize($condition['SITES'], ['allowed_classes' => false])));?>
        </td>
    </tr>
    ...
    <tr>
        ...
        <td>
            <?=SelectBoxMFromArray('SECTIONS[]', $arrSectionsList ?: array(), is_array($condition['SECTIONS']) ? $condition['SECTIONS'] : unserialize($condition['SECTIONS'], ['allowed_classes' => false]), '', false, '', 'style="min-width: 350px;"');?>
        </td>
    </tr>
    ...
    <tr>
        ...
        <td>
            <select class="js-select2" name="CATEGORY">
                ...
                <?
                $RsAllSectionsCHPU = SitemapSectionTable::getList([
                    'select' => ['*'],
                    'filter' => ['ACTIVE' => 'Y'],
                    'order' => ['SORT' => 'ASC']
                ])->fetchAll();
                foreach ($RsAllSectionsCHPU as $section) {
                    $selected = '';
                    if ($condition['CATEGORY'] === $section['ID']) {
                        $selected = 'selected';
                    } ?>
    ...
    <tr>
        <td width="40%">
            <?=Loc::getMessage("SEOMETA_FIELD_TAGS_CATEGORY")?>
        </td>
        <td width="60%">
            <select class="js-select2" name="META[CATEGORY_TAGS_SECTION]">
                <option value="0"><?= Loc::getMessage("SEO_META_CHECK_CATEGORY_TAGS") ?></option>
                <?
                ...
                foreach ($categoryTags as $section) {
                    $selected = '';
                    if ($meta['CATEGORY_TAGS_SECTION'] === $section['ID']) {
                        $selected = 'selected';
                    } ?>
                    <option value='<?= $section['ID'] ?>' <?= $selected ?>><?= $section['NAME'] ?></option>
                <? } ?>
    ...
    <tr>
        <td width="40%">
            <?=Loc::getMessage("SEOMETA_FIELD_TAGS_DETAIL_CATEGORY")?>
        </td>
        <td width="60%">
            <select class="js-select2" name="META[CATEGORY_TAGS_DETAIL_SECTION]">
                <option value="0"><?= Loc::getMessage("SEO_META_CHECK_CATEGORY_TAGS") ?></option>
                <?
                $categoryTags = \Sotbit\Seometa\Orm\CategoryTagsTable::getList([
                    'select' => ['*'],
                ...
                foreach ($categoryTags as $section) {
                    $selected = '';
                    if ($meta['CATEGORY_TAGS_DETAIL_SECTION'] === $section['ID']) {
                        $selected = 'selected';
                    } ?>
                    <option value='<?= $section['ID'] ?>' <?= $selected ?>><?= $section['NAME'] ?></option>
                <? } ?>
    ...
    <tr>
        <td><?=Loc::getMessage("SEOMETA_FIELD_TAGS_DETAIL")?></td>
        <td>
            <textarea name="META[ELEMENT_TAG]" id="META[ELEMENT_TAG]" cols="55" rows="1" style="width: 90%;"><?=$meta['ELEMENT_TAG'] ?: ''?></textarea>
        </td>
        <td width="10%"
            align="left">
            <?= $PropMenu ?>
