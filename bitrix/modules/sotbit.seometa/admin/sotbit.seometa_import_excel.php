...
?>
    ...
    <tr class="adm-detail-file-row">
        <td width="40%">
            <span data-hint="<?= Loc::getMessage($id_module . '_WARNING'); ?>"></span>
            <?= $tabControl->GetCustomLabelHTML(); ?></td>
        <td width="60%"><?= FileInput::createInstance([
...
if ($arColumn) {
    ...
    <tr id="CATEGORY_ID">
        ...
        <td width="60%">
            <select class="js-select2" name="CATEGORY_ID">
                <option value="0"><?= Loc::getMessage($id_module. "_CHOOSE_CATEGORY") ?></option>
                <?
                foreach ($catValues as $section) {
                    $selected = '';
                    if ($request->get('CATEGORY_ID') === $section['ID']) {
                        $selected = 'selected';
                    } ?>
    ...
    if ($entity !== 'cond') {
        $tabControl->BeginCustomField('MODE', Loc::getMessage($id_module . '_MODE'));
        ?>
        <tr id="CATEGORY_ID">
            <td width="40%">
                <span data-hint="<?= Loc::getMessage($id_module . '_MODE_HINT'); ?>" data-hint-html></span>
                <?= $tabControl->GetCustomLabelHTML() ?></td>
            <td width="60%">
                <select name="MODE">
                    <?
                    foreach ([
                                 ImportHelper::MODE_CREATE,
                                 ImportHelper::MODE_UPDATE,
                             ] as $mode) {
                        $selected = '';
                        if ($request->get('MODE') === $mode) {
                            $selected = 'selected';
                        } ?>
                        <option value='<?= $mode ?>' <?= $selected ?>><?= Loc::getMessage($id_module . '_MODE_' . $mode) ?></option>
                    <? }
                    ?>
                </select>
            </td>
        ...
        <?
        $tabControl->EndCustomField('MODE');
    }
...
$tabControl->Show(); ?>
    <script>
        $(document).ready(function() {
            ...
            $('.js-select2').select2({
                ...
                language: {
                    'noResults': function(){
                        return '<?= Loc::getMessage($id_module . "_CHOOSE_CATEGORY_NO_FOUND") ?>';
                    }
                }
            });
        });
    ...
    <script>
        ...
        document.querySelector('#bx_file_element_file_upload_add').addEventListener('click', function (){
            setTimeout(function () {
                document.querySelector('.adm-menu-download').parentNode.style.display = 'none';
            }, 1);
