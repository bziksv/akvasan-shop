...
if (is_array($meta) && !empty($chpu['PROPERTIES'])) {
    SeoMetaMorphy::init($chpu['section_id'], unserialize($chpu['PROPERTIES']));

    foreach ($meta as &$item) {
        if (!is_array($item) && mb_strpos($item, '{') !== false) {
            $item = SeoMetaMorphy::processMorphy($item);
        }
    }
}
...
if ($request->get('action')) {
    if ($request->get('action') == "copy" && $ID > 0) {
        ...
        $linkChpuData = SeometaUrlTable::getById($request->get('chpu_id'));
        $result = ChpuLinksTable::add([
            'MAIN_CHPU_ID' => $ID,
            'LINK_CHPU_ID' => intval($request->get('chpu_id')),
            'MAIN_REAL_URL' => $chpu['REAL_URL'],
            'LINK_REAL_URL' => $linkChpuData['REAL_URL'],
            'SITE_ID' => $chpu['SITE_ID'],
...
if ($request->getRequestMethod() === "POST" && ($request->get('save') || $request->get('apply')) && $rights == "W" && check_bitrix_sessid()) {
    ...
    if ($request->get('SEOMETA_DATA')['ELEMENT_FILE'] && $request->get('SEOMETA_DATA_del')['ELEMENT_FILE'] == 'Y') {
        ...
        if (!is_numeric($file)) {
            if (is_array($file)) {
                $file['tmp_name'] = CTempFile::GetAbsoluteRoot() . $file['tmp_name'];
            } else {
...
<tr id="CATEGORY_ID">
    ...
    <td width="60%">
        <select class="js-select2" name="CATEGORY_ID">
            <option value="0"><?= Loc::GetMessage("SEO_META_CHECK_CATEGORY") ?></option>
            <?
            ...
            foreach ($RsAllSections as $section) {
                $selected = '';
                if ($chpu['CATEGORY_ID'] === $section['ID']) {
                    $selected = 'selected';
                } elseif (!$chpu['CATEGORY_ID'] && $request->get('section') === $section['ID']) {
                    $selected = 'selected';
                } ?>
...
<tr class="adm-detail-valign-top">
    ...
    <td width="60%">
        ...
        <? if ($chpu['SITE_ID'] && $chpu['REAL_URL'] && $siteInfo['SERVER_NAME']): ?>
            <a style="vertical-align: top !important;" class="adm-btn"
               href="<?= $protocol . $siteInfo['SERVER_NAME'] . $chpu['REAL_URL']; ?>"><?= Loc::getMessage('SEO_META_EDIT_CHECK_URL'); ?></a>
        <? endif; ?>
    </td>
<tr>
...
<tr class="adm-detail-valign-top">
    ...
    <td width="60%">
        ...
        <? if ($chpu['SITE_ID'] && $chpu['NEW_URL'] && $siteInfo['SERVER_NAME']): ?>
            <a style="vertical-align: top !important;" class="adm-btn adm-detail-valign-top"
               href="<?= $protocol . $siteInfo['SERVER_NAME'] . $chpu['NEW_URL']; ?>"><?= Loc::getMessage('SEO_META_EDIT_CHECK_URL'); ?></a>
        <? endif; ?>
    </td>
<tr>
    <?php $tabControl->EndCustomField('NEW_URL');
    $tabControl->BeginCustomField("CONDITION_ID", loc::getMessage('SEO_META_EDIT_CONDITION_ID')); ?>
<tr id="tr_CONDITION_ID">
    <td width="40%"><?= $tabControl->GetCustomLabelHTML(); ?></td>
    <td width="60%">
        <select class="js-select2" name="CONDITION_ID">
            <?
            unset($item);
            foreach ($chpus as $key => $item) {
                $selected = '';
                if ((int)$chpu['CONDITION_ID'] === $key) {
                    $selected = 'selected';
                } ?>
...
<? $tabControl->EndCustomField("CONDITION_ID");

$tabControl->BeginCustomField("SITE_ID_AREA", loc::getMessage('SEO_META_EDIT_SITES'), true);
?>
...
if ($ID) {
    $iblock = CIBlock::GetByID($chpu['iblock_id'])->fetch();
    $tabControl->AddViewField('IBLOCK_ID', Loc::getMessage('SEO_META_EDIT_IBLOCK'), $iblock ? ('<a target="_blank" href="iblock_edit.php?type=' . $iblock['IBLOCK_TYPE_ID'] . '&lang=' . LANG . '&ID=' . $chpu['iblock_id'] . '&admin=Y">' . $iblock['NAME'] . '</a>') : '—');
    if ($chpu['section_id'] === '0') {
        $sectionLink = '<span>'.Loc::getMessage('SEO_META_EDIT_SECTION_TOP_LEVEL').'</span>';
    } else {
        $section = CIBlockSection::GetByID($chpu['section_id'])->fetch();
        $sectionLink = $section ? ('<a target="_blank" href="iblock_section_edit.php?IBLOCK_ID=' . $chpu['iblock_id'] . '&type=' . $iblock['IBLOCK_TYPE_ID'] . '&ID=' . $chpu['section_id'] . '&lang=' . LANG . '&find_section_section=' . $section['IBLOCK_SECTION_ID'] . '">' . $section['NAME'] . '</a>') : '—';
    }
    $tabControl->AddViewField('SECTION_ID', Loc::getMessage('SEO_META_EDIT_SECTION'), $sectionLink);
    $tabControl->AddViewField('PRODUCT_COUNT', Loc::getMessage('SEO_META_EDIT_PRODUCT_COUNT'), $chpu['SEO_META_EDIT_PRODUCT_COUNT']);
    $tabControl->AddViewField('PROPERTIES', Loc::getMessage('SEO_META_EDIT_PROPERTIES'), $chpu['PROPERTIES'] ? \Sotbit\Seometa\Helper\AdminSection\Tools::getPropertyChpuValue($chpu['PROPERTIES'], $chpu['iblock_id']) : '—');
}
...
if ($ID > 0) {
    ...
    <tr id="tr_LISTCHPU">
        <td colspan="2">
            <a class="adm-btn adm-btn-add"
               onclick="jsUtils.OpenWindow('/bitrix/admin/templates/chpu_list.php<?= $urlParams ?>', 900, 700);"><?= Loc::getMessage('SEO_META_EDIT_ADD_LINK') ?></a>
        </td>
    </tr>
    <?php
                    ...
                    <td>
                        <div class="graph-item">
                            <div class="graph-item__title">
                                <div class="graph-item__title-collapse"
...
$aiInc = \Bitrix\Main\Config\Option::get(\CCSeoMeta::MODULE_ID, 'AI_FUNC_INC') === 'Y';
...
<script>
    const messages = {
        title: "<?= Loc::getMessage('SEO_META_EDIT_NAME') ?> "
    };
    ...
    <script>
        MessageForRequest = function () {
            this.arSeo = {
                ...
                'UPPER_DESCRIPTION': document.querySelector('#ELEMENT_TOP_DESC'),
                'LOWER_DESCRIPTION': document.querySelector('#ELEMENT_BOTTOM_DESC'),
                'ADDITIONAL_DESCRIPTION': document.querySelector('#ELEMENT_ADD_DESC'),
            }
