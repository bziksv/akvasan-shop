...
Class sotbit_seometa extends CModule
{
	...
	function InstallEvents()
	{
		...
		RegisterModuleDependences("seo", "\Bitrix\Seo\Sitemap\Internals\Sitemap::OnAfterUpdate", self::MODULE_ID, "CSeoMetaEvents", "GenerateSeoMetaSitemap");
	...
	function UnInstallEvents()
	{
		...
		UnRegisterModuleDependences("seo", "\Bitrix\Seo\Sitemap\Internals\Sitemap::OnAfterUpdate", self::MODULE_ID, "CSeoMetaEvents", "GenerateSeoMetaSitemap");
	...
	function installDefaultTagsCategory()
	{
		\Bitrix\Main\Loader::includeModule(self::MODULE_ID);

		$defaultTag = \Sotbit\Seometa\Orm\CategoryTagsTable::query()
			->setFilter(['ID' => 1])
			->addSelect('*')
			->fetch();

		if(!$defaultTag) {
			\Sotbit\Seometa\Orm\CategoryTagsTable::add(
				[
					'ACTIVE' => 'Y',
					'SORT' => 9999,
					'NAME' => Loc::getMessage('SOTBIT_SEOMETA_INSTALL_TAGS_CATEGORY_DEFAULT_NAME'),
				]
			);
		}
	}
	...
	function DoInstall()
	{
		...
		if(version_compare(phpversion(), '7.4', '<')) {
			$APPLICATION->ThrowException(Loc::getMessage('SOTBIT_SEOMETA_INSTALL_ERROR_VERSION_PHP'));
			return false;
		}
			...
			if (!empty($_REQUEST['Email']) && !empty($_REQUEST['Name']) && !empty($_REQUEST['Phone'])) {
				if (filter_var($_REQUEST['Email'], FILTER_VALIDATE_EMAIL)) {
					if (preg_match("/^[0-9()+\-\s]+$/", $_REQUEST['Phone'])) {
						...
						if (method_exists(\CUpdateClientPartner::class, "GetLicenseKey")) {
							$request['KEY'] = md5("BITRIX" . \CUpdateClientPartner::GetLicenseKey() . "LICENCE");
						} else {
							$request['KEY'] = md5("BITRIX". \Bitrix\Main\Application::getInstance()->getLicense()->getKey() . "LICENCE");
						}
						...
						$this->installDefaultTagsCategory();
                    } else {
