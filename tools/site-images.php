...
class LormagSiteImages
{
	...
	public const PUBLIC_BASE = 'https://akvasan-shop.ru';
	...
	{
		$filename = 'lormag-images-' . date('Y-m-d-His') . '.xls';
...
if (!LormagSiteImages::isAllowed()) {
	...
	echo '<body style="font:14px/1.5 system-ui;padding:40px"><h1>Доступ запрещён</h1><p>'
		. htmlspecialchars(LormagSiteImages::accessDeniedReason())
		. '</p></body></html>';
	exit;
}
...
if ($action === 'download') {
	LormagSiteImages::downloadFile((int)($_GET['id'] ?? 0));
}
...
$limit = max(1, min(LormagSiteImages::PAGE_SIZE_MAX, (int)($_GET['limit'] ?? LormagSiteImages::PAGE_SIZE_DEFAULT)));
$offset = ($page - 1) * $limit;
$export = isset($_GET['export']) && $_GET['export'] === 'excel';

$result = LormagSiteImages::fetch([
	'q' => $q,
	'module' => $module,
	'usage' => $usage,
	'limit' => $limit,
...
if ($export) {
	LormagSiteImages::exportExcel($result['rows']);
}

$total = $result['total'];
$rows = $result['rows'];
$pages = max(1, (int)ceil($total / $limit));
$modules = LormagSiteImages::modulesList();
...
<head>
	...
	<title>Картинки сайта — lormag.ru</title>
	<style>
		:root {
			--bg: #f0f3f7;
			--card: #fff;
				...
				<tr>
					...
					<td>
						...
						<div style="color:var(--muted);font-size:12px;margin-top:4px">
							...
							<?=htmlspecialchars(LormagSiteImages::formatSize((int)$r['FILE_SIZE']))?>
