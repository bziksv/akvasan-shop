...
<div class="additions-product"><div class="row">
    <div class="checkbox-area col-lg-1 col-md-1 col-xs-1"><div class="container-checkbox">
        <?if($arParams['CHECKBOX'][$key] == 'да'):?>
            <input type="checkbox" id="check<?=$arItem['ID']?>" value="<?=$arItem['ID']?>" checked='checked'>
        <?else:?>
    ...
    <div class="img-additions-product col-lg-2 col-md-2 col-xs-2">
        ...
        <?else:?>
        <div class="container-img"><a href="<?=$arItem['DETAIL_PAGE_URL']?>"><img src="/upload/template/no_photo_200.png" alt="<?=$arItem['NAME']?>"></a></div>
        <?endif?>
    </div>
    <div class="info-additions-product col-lg-6 col-md-6 col-xs-6">
        <div class="title-info"><a href="<?=$arItem['DETAIL_PAGE_URL']?>"><?=$arItem['NAME']?></a></div>
    ...
    <div class="price-additions-product col-lg-2 col-md-2 col-xs-2">
        <span><?=str_replace(" руб.","",$arItem["ITEM_PRICES"][$arItem["ITEM_PRICE_SELECTED"]]["PRINT_BASE_PRICE"])?> р.</span>
    </div>
</div></div>

<input type='hidden' data-price-complect='<?=$arItem["ID"]?>' value='<?=$arItem["ITEM_PRICES"][$arItem["ITEM_PRICE_SELECTED"]]['BASE_PRICE']?>' />
<?endforeach?>