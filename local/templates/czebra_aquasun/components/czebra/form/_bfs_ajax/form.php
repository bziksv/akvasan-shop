                ...
                switch ($type):
                    ...
                    case "radio":
                        foreach ($arResult["PROPERTY_LIST_FULL"][$propertyID]["ENUM"] as $key => $arEnum) {
                            ...
                            ?>
                                <input  <?=$validation?> type="<?=$type?>" name="PROPERTY[<?=$propertyID?>]<?=$type == "checkbox" ? "[".$key."]" : ""?>" value="<?=$key?>" id="property_<?=$key?>"<?=$checked ? " checked=\"checked\"" : ""?> /><label for="property_<?=$key?>"><?=$arEnum["VALUE"]?></label><br />
                            <?
                        }
                    break;
                    case "dropdown":
...
<?if($arParams["VALIDATED"] == "Y"):?>
<script>
cz_validated.run('<?=$arParams["FORM_ID"]?>_sibmit', '<?=$arParams["FORM_ID"]?>_group');
</script>
<?endif?>