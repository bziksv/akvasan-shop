...
(function () {
	...
	function noticeAnchor(inp) {
		var authField = inp.closest('.auth-form__field');
		if (authField) return authField;
		var orderLi = inp.closest('form[name="ORDER_FORM"] li, #ORDER_FORM li, .cart-info li');
		if (orderLi && orderLi.contains(inp)) return orderLi;
		var bxGroup = inp.closest('.bx-authform-formgroup-container');
		if (bxGroup) return bxGroup;
		var td = inp.closest('td');
		if (td && inp.closest('form[name="bform"], table.bx-registration-table, table.bx-auth-table')) {
			return td;
		}
	...
	function contextFor(inp) {
		...
		if (inp.getAttribute('data-prime-alerts-email') === '1'
			&& inp.closest('form[name="ORDER_FORM"], #ORDER_FORM, .order-checkout, .container-order')) {
			return 'checkout';
		}
	...
	function ensureBox(inp, create) {
		...
		var insideAnchor = anchor.contains && anchor.contains(inp);
		if (insideAnchor) {
			var nested = anchor.querySelector(':scope > .prime-alerts-live-notice');
			if (nested) return nested;
			if (!create) return null;
			nested = document.createElement('div');
			nested.className = 'prime-alerts-live-notice';
	...
	function dismissNoticeBox(inp, kind) {
		var box = ensureBox(inp, false);
		if (!box) return;
		var boxKind = box.getAttribute('data-kind') || '';
		if (kind && boxKind && boxKind !== kind) return;
		hideBox(box);
		box.innerHTML = '';
	...
	function isEmailInput(inp) {
		...
		if (inp.getAttribute('data-prime-alerts-email') === '1') return true;
		...
		if (/^order_prop_/i.test(name) && inp.closest('form[name="ORDER_FORM"], #ORDER_FORM') && (type === 'email' || auto === 'email')) {
			return true;
		}
		if (id.indexOf('email') >= 0) return true;
		if (/e-?mail|эл\.?\s*почт|электрон/i.test(placeholder)) return true;
		var wrap = fieldWrap(inp);
	...
	function refreshInput(inp, opts) {
		...
		if (cfg.enabled === false || !ctx || !policyEnabledFor(ctx)) {
			if (!(ctx === 'signup' && cfg.checkEmailDuplicate !== false && duplicateStateFor(inp).exists)) {
				dismissNoticeBox(inp, 'policy');
			}
			return;
		}
	...
	function scan(root) {
		...
		initOrderFormEmail();
		var nodes = root.querySelectorAll('input[type="email"], input[type="text"], input:not([type]), input[data-prime-alerts-email="1"]');
	...
	function initOrderFormEmail() {
		var inputs = document.querySelectorAll('form[name="ORDER_FORM"] input[data-prime-alerts-email="1"], #ORDER_FORM input[data-prime-alerts-email="1"]');
		for (var i = 0; i < inputs.length; i++) {
			bindOrderEmailInput(inputs[i]);
		}
	}

	function bindOrderEmailInput(inp) {
		if (!inp || inp.getAttribute('data-prime-order-bound') === '1') return;
		inp.setAttribute('data-prime-order-bound', '1');
		var lastVal = '';
		function runCheck() {
			if (looksComplete(inp.value)) refreshInput(inp, { force: true });
		}
		['input', 'change', 'blur', 'keyup', 'paste'].forEach(function (ev) {
			inp.addEventListener(ev, runCheck);
		});
	...
	function initBxRegistrationEmail() {
		var inp = document.querySelector('form[name="bform"] input[name="USER_EMAIL"]')
			|| document.querySelector('.bx-authform form[name="bform"] input[name="USER_EMAIL"]');
	...
	window.primeAlertsCheckRegistrationEmail = function (inp) {
		if (!inp) {
			inp = document.querySelector('form[name="bform"] input[name="USER_EMAIL"]')
				|| document.querySelector('.bx-authform form[name="bform"] input[name="USER_EMAIL"]');
		}
