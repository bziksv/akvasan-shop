...
$APPLICATION->SetTitle("Каталог товаров - страны");
...
$brand = strtolower($request["country"]);
if (strpos($brand, "?")) {
...
if (strlen($brand) > 0) {
    $propFilter = array();                
    $propertyEnums = CIBlockPropertyEnum::GetList(Array("value" => "ASC", "SORT" => "ASC"), Array("IBLOCK_ID" => $iblock, "CODE" => "STRANA%"));
    ...
    $cache = new CPHPCache();
    $cache_time = 36000;
    $cache_id = 'listCountryCzebra';
    $cache_path = '/listBrandCzebra/';
    if ($cache_time > 0 && $cache->InitCache($cache_time, $cache_id, $cache_path)) {
        $res = $cache->GetVars();
        if (is_array($res["arFirstCharCountry"]) && (count($res["arFirstCharCountry"]) > 0)) {
            $arFirstChar = $res["arFirstCharCountry"];
        }
        if (is_array($res["listBrandCountry"]) && (count($res["listBrandCountry"]) > 0)) {
            $listBrand = $res["listBrandCountry"];
        }
    }
    ...
    if (!is_array($listBrand) && !is_array($arFirstChar)) {
        ...
        while($enum_fields = $propertyEnums->GetNext()) {
            $count = CIBlockElement::GetList([], ["IBLOCK" => $iblock, "ACTIVE" => "Y", "PROPERTY_STRANA" => $enum_fields["ID"]], false, ["nPageSize"=>1], ["ID", "IBLOCK"]);
        ...
        if ($cache_time > 0) {
            ...
            $cache->EndDataCache(array(
                "arFirstCharCountry" => $arFirstChar,
                "listBrandCountry" => $listBrand,
            ));
        }
    }
}
        ...
        <div class="container">
            ...
            <?foreach($listBrand as $key => $arItem):?>
                ...
                <div class="item-brand-card" data-search-char="<?=$arItem["FIRST_CHAR"]?>">
                    <a href="/country/<?=$arItem["CODE"]?>/">
                        <span class="card-img-brand">
                            <?
                            $file = "/upload/template_new/flag_" . Cutil::translit($arItem["NAME"],"ru",array()) . ".png";
...
<?
    ...
    <div class="container-sorting">
        <span class="title-sorting">Страна - <?=$name?></span>
